...
public class ReactiveForwarding {
    ...
    private final String pathFileCSV = "/home/vantv/Desktop/routingserver/collection/collect.csv";
    private final float TOTAL_RATE = (float)1/(float)3;
    private final float LOSS_PACKETs_PERCENT = (float)1/(float)3;
    private final float LATENCY = 1 - TOTAL_RATE - LOSS_PACKETs_PERCENT;

    private class Info {
        long port1; String deviceId2; long port2; float cost;
        public Info(long port1, String deviceId2, long port2, float cost) {
            this.port1 = port1;
    ...
    private class Info1 {
        long port; float cost;  
        public Info1(long port, float cost) {
            this.port = port; 
            this.cost = cost;
        }
    }
    ...
    private class Info2 {
        String deviceId; float cost;
        public Info2(String deviceId, float cost) {
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
            ...
            String srcDeviceId = pkt.receivedFrom().deviceId().toString();
            String dstDeviceId = dst.location().deviceId().toString();
            long   srcPort     = context.inPacket().receivedFrom().port().toLong(); // port ra srcDeviceId phai != port nay
            long   dstPort     = dst.location().port().toLong();                    // port vao dstDeviceId phai != port nay
            boolean succesReadfile;
            ArrayList<String> lines = new ArrayList<String>();

            try {
                Scanner sc = new Scanner(new File(pathFileCSV));
                // line name of cols: srcDeviceId,srcPort,dstDeviceId,dstPort,totalRate,lossPkt,latency
                if (sc.hasNextLine()) log.info(sc.nextLine());

                while (sc.hasNextLine()) lines.add(sc.nextLine());

                succesReadfile = true;
                // log.info("##########----------ReactivePacketProcessor Read File Ok----------########\n");
            }
            catch (Exception e) {succesReadfile = false;}
            
            if (succesReadfile && lines.size() > 0) {
                ArrayList<Float> ttalRates = new ArrayList<>();
                ArrayList<Float> lossPkpts = new ArrayList<>();
                ArrayList<Float> latencies = new ArrayList<>();

                for (String line: lines) {
                    String[] arr = line.split(",");
                    ttalRates.add(Float.valueOf(arr[4]));
                    lossPkpts.add(Float.valueOf(arr[5]));
                    latencies.add(Float.valueOf(arr[6]));
                }

                //calculate distance, min of totalRate, lossPacketsPercent, latency
                float maxTtlRate = 0;
                float maxLossPkt = 0;
                float maxLatency = 0;

                for (int i = 0; i < lines.size(); i++) {
                    if (ttalRates.get(i) > maxTtlRate) maxTtlRate = ttalRates.get(i);
                    if (lossPkpts.get(i) > maxLossPkt) maxLossPkt = lossPkpts.get(i);
                    if (latencies.get(i) > maxLatency) maxLatency = latencies.get(i);
                }

                float minTtlRate = maxTtlRate;
                float minLossPkt = maxLossPkt;
                float minLatency = maxLatency;

                for (int i = 0; i < lines.size(); i++) {
                    if (ttalRates.get(i) != -1 && ttalRates.get(i) < minTtlRate) minTtlRate = ttalRates.get(i);
                    if (lossPkpts.get(i) != -1 && lossPkpts.get(i) < minLossPkt) minLossPkt = lossPkpts.get(i);
                    if (latencies.get(i) != -1 && latencies.get(i) < minLatency) minLatency = latencies.get(i);
                }

                float distanceTtlRate = maxTtlRate - minTtlRate + (float)(1e-8);
                float distanceLossPkt = maxLossPkt - minLossPkt + (float)(1e-8);
                float distanceLatency = maxLatency - minLatency + (float)(1e-8);

                //Info {port1,device2,port2,cost}
                Map<String, ArrayList<Info>> mapDeviceInfo = new HashMap<String, ArrayList<Info>>();

                for (int i = 0; i < lines.size(); i++) {

                    String[] arr = lines.get(i).split(",");

                    if (arr[0] == srcDeviceId &&  Long.valueOf(arr[1]) == srcPort) continue;
                    if (arr[2] == srcDeviceId &&  Long.valueOf(arr[3]) == srcPort) continue;
                    if (arr[0] == dstDeviceId &&  Long.valueOf(arr[1]) == dstPort) continue;
                    if (arr[2] == dstDeviceId &&  Long.valueOf(arr[3]) == dstPort) continue;

                    String deviceId1 = arr[0];
                    String deviceId2 = arr[2];
                    long   port1     = Long.valueOf(arr[1]);
                    long   port2     = Long.valueOf(arr[3]);

                    float cost = 2;

                    float latency = latencies.get(i);
                    float ttlRate = ttalRates.get(i);
                    float lossPkt = lossPkpts.get(i);
                    
                    if (latency != -1 && ttlRate != -1 && lossPkt != -1) {
                        //cost always in [0, 1]
                        cost  = (ttlRate - minTtlRate) / distanceTtlRate * TOTAL_RATE
                            + (lossPkt - minLossPkt) / distanceLossPkt * LOSS_PACKETs_PERCENT
                            + (latency - minLatency) / distanceLatency * LATENCY;
                    }
                    
                    if (!mapDeviceInfo.containsKey(deviceId1)) mapDeviceInfo.put(deviceId1, new ArrayList<Info>());
                    ArrayList tmp1 = mapDeviceInfo.get(deviceId1);
                    tmp1.add(new Info(port1, deviceId2, port2, cost));
                    mapDeviceInfo.replace(deviceId1, tmp1);
                }

                //dijkstra():
                //Info1 {port,cost} : port: port form deviceId to dstDeviceId, cost: cost form deviceId to dstDeviceId
                //Map<deviceId, {port,cost}> : has path into dstDeviceId
                Map<String, Info1> paths = new HashMap<String, Info1>();
                //Info2 {deviceId,cost} : cost: cost form deviceId to dstDeviceId
                PriorityQueue<Info2> priorityQueue = new PriorityQueue<Info2>(new Comparator<Info2>() {
                    @Override
                    public int compare(Info2 o1, Info2 o2) {return Float.compare(o1.cost, o2.cost);}
                });
                int count = 0;

                paths.remove(dstDeviceId);
                paths.put(dstDeviceId, new Info1(0, 0));
                priorityQueue.add(new Info2(dstDeviceId, 0));

                while (priorityQueue.size() > 0 && count < 100000) {
                    String device1 = priorityQueue.poll().deviceId;

                    if (!mapDeviceInfo.containsKey(device1)) continue;
                    count++;
                
                    for (Info l: mapDeviceInfo.get(device1)) {
                        float cost1 = paths.get(device1).cost;
                        float cost2;

                        if (! paths.containsKey(l.deviceId2)) {
                            cost2 = l.cost + cost1;
                            paths.put(l.deviceId2, new Info1(l.port2, cost2));
                            priorityQueue.add(new Info2(l.deviceId2, cost2));
                        }
                        else if (l.cost + cost1 < paths.get(l.deviceId2).cost) {
                            cost2 = l.cost + cost1;
                            paths.remove(l.deviceId2);
                            paths.put(l.deviceId2, new Info1(l.port2, cost2));
                            priorityQueue.add(new Info2(l.deviceId2, cost2));
                        }
                    }
                ...
                if (paths.containsKey(srcDeviceId)) {                    
                    String deviceId3 = installRule(context, PortNumber.portNumber(paths.get(srcDeviceId).port), macMetrics);

                    log.info("ReactivePacketProcessor LinkState:\n{} \\ {} -> {} # {}", 
                                pkt.receivedFrom().deviceId(), 
                                paths.get(srcDeviceId).port ,
    ...
    private String installRule(PacketContext context, PortNumber portNumber, ReactiveForwardMetrics macMetrics) {
        //
        ...
        if (packetOutOnly || inPkt.getEtherType() == Ethernet.TYPE_ARP) {
            ...
            return context.inPacket().receivedFrom().deviceId().toString();
        }
        ...
        // FlowRule r = DefaultFlowRule.builder().forDevice(context.inPacket().receivedFrom().deviceId())
        //         .withSelector(selectorBuilder.build())
        //         .withTreatment(treatment)
        //         .withPriority(flowPriority)
        //         .fromApp(appId) 
